...
export default function DashboardTabs({ statusProses }: DashboardTabsProps) {
  ...
  return (
    <div className="w-full bg-white/80 backdrop-blur-md sticky top-0 z-30 border-b border-amber-100 shadow-sm overflow-hidden rounded-b-[2rem]">
          ...
          return (
            <Link
              ...
              className={`flex-shrink-0 flex items-center gap-2 px-4 py-3 rounded-2xl transition-all duration-300 relative group ${
                isActive
                  ? "bg-blue-900 text-white shadow-blue-200 shadow-lg scale-105"
                  : "text-ink-600 hover:bg-amber-50 hover:text-blue-900"
              }`}
            >
              <Icon
                className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform group-hover:scale-110 ${isActive ? "text-amber-300" : ""}`}
              />
              <span
