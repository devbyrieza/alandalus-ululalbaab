...
export default function KesiapanTestPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const saved = localStorage.getItem("alandalus_ululalbaab_ujian_kesiapan_draft");
        if (saved) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && Object.keys(answers).length > 0) {
      try {
        localStorage.setItem("alandalus_ululalbaab_ujian_kesiapan_draft", JSON.stringify(answers));
      } catch (e) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        localStorage.removeItem("alandalus_ululalbaab_ujian_kesiapan_draft");
      }
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-6 pb-24">
      ...
      <div className="sticky bottom-4 z-50">
        <button
          ...
          disabled={loading}
          className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 disabled:bg-stone-300 text-white font-bold rounded-lg shadow-sm transition-colors flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
