...
export default function AkademikTestPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const saved = localStorage.getItem("alandalus_ululalbaab_ujian_akademik_draft");
        if (saved) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && Object.keys(answers).length > 0) {
      try {
        localStorage.setItem("alandalus_ululalbaab_ujian_akademik_draft", JSON.stringify(answers));
      } catch (e) {
  ...
  const handleSubmit = async (autoSubmit = false) => {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        localStorage.removeItem("alandalus_ululalbaab_ujian_akademik_draft");
      }
  ...
  if (!started) {
    return (
      <div className="max-w-3xl mx-auto p-4 md:p-6">
        ...
        <div className="bg-gradient-to-r from-primary-600 to-primary-800 rounded-[2rem] p-5 md:p-8 mb-8 text-white relative overflow-hidden shadow-sm border border-primary-500">
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6 pb-32">
      {/* Floating Timer */}
      <div
        className={`fixed top-20 right-4 md:right-10 bg-white border shadow-sm rounded-full px-4 py-2 z-50 flex items-center gap-2 font-bold font-mono text-xl ${timeLeft <= 300 ? "text-red-600 animate-pulse" : "text-indigo-600"}`}
      >
