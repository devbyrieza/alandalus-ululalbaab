...
const StatWidget = ({ label, value, icon: Icon, color, trend, breakdown }: any) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      className="bg-white p-8 rounded-[2.5rem] border border-brand-blue-100 shadow-premium-sm hover:shadow-premium-lg transition-all group relative overflow-hidden">
      ...
      <div className="relative z-10">
        ...
        <div className="mb-8">
          <p className="text-[11px] font-black text-brand-blue-400 uppercase tracking-[0.25em] mb-2">{label}</p>
          <div className="flex items-baseline gap-2">
            <h3 className="text-5xl font-black text-brand-blue-900 tracking-tighter italic">{value}</h3>
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-8 pb-20">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-brand-blue-900 tracking-tight italic">Dasbor Ulul Albaab</h1>
          <p className="text-sm text-brand-blue-500 font-medium mt-1">Pantau perkembangan pendaftaran santri secara langsung.</p>
        </div>
        <button onClick={fetchStats} className="p-3 bg-white border border-brand-blue-100 rounded-2xl text-brand-blue-400 hover:text-brand-blue-600 transition-all shadow-premium-sm">
          <RefreshCw className={`w-5 h-5 ${loading ? "animate-spin" : ""}`} />
        </button>
      </div>

      <div className="bg-brand-blue-950 rounded-[3rem] p-12 text-white relative overflow-hidden shadow-2xl border border-white/5">
          ...
          <div className="max-w-xl">
            ...
            <div className="flex items-center gap-12 mt-12">
              <div>
                ...
                <p className="text-[11px] font-black text-brand-blue-200 uppercase tracking-widest mt-2">Pendaftar</p>
              </div>
              <div className="w-px h-16 bg-white/20" />
              <div>
                <span className="text-5xl font-black text-brand-yellow-400 italic">{stats.sudah_bayar}</span>
                <p className="text-[11px] font-black text-brand-blue-200 uppercase tracking-widest mt-2">Tervalidasi</p>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-6 w-full lg:w-auto">
            <div className="bg-white/5 backdrop-blur-md p-8 rounded-[2.5rem] border border-white/10 flex flex-col items-center">
              <p className="text-[10px] font-black text-brand-blue-200 uppercase tracking-widest mb-4">Lulus Seleksi</p>
              <p className="text-4xl font-black text-emerald-400 italic">{stats.diterima}</p>
            </div>
            <div className="bg-white/5 backdrop-blur-md p-8 rounded-[2.5rem] border border-white/10 flex flex-col items-center">
              <p className="text-[10px] font-black text-brand-blue-200 uppercase tracking-widest mb-4">Daftar Ulang</p>
              <p className="text-4xl font-black text-brand-yellow-400 italic">{stats.daftar_ulang}</p>
            </div>
          </div>
        </div>
      </div>
