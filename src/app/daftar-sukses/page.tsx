...
function DaftarSuksesContent() {
  ...
  return (
    <div className="app-card max-w-lg w-full bg-white rounded-[2.5rem] shadow-lg border border-brand-blue-200 p-8 relative overflow-hidden">
      {/* Background Blurs */}
      <div className="absolute -top-32 -right-32 w-64 h-64 bg-brand-blue-50 rounded-full blur-3xl pointer-events-none" />

      {/* Success Icon */}
      <div className="text-center relative z-10 mb-6 mt-4">
        <div className="relative inline-flex items-center justify-center w-24 h-24 bg-brand-blue-50 rounded-3xl border border-brand-blue-200">
          <CheckCircle2 className="w-14 h-14 text-brand-blue-600 animate-bounce" />
          <Sparkles className="w-8 h-8 text-brand-blue-200 absolute -top-3 -right-3 animate-pulse" />
        </div>
      </div>
      ...
      <div className="text-center relative z-10 mb-8">
        <h1 className="text-3xl font-display font-black text-ink-950 mb-2 flex items-center justify-center gap-2">
          <PartyPopper className="w-6 h-6 text-brand-blue-600" />
          Alhamdulillah!
        </h1>
      ...
      {/* Credentials Box */}
      <div className="bg-brand-blue-50 border border-brand-blue-200 rounded-[1.5rem] p-6 mb-6 relative z-10">
        <p className="text-[10px] font-black tracking-widest uppercase text-brand-blue-600 mb-4 text-center flex items-center justify-center gap-2">
          <CreditCard className="w-3.5 h-3.5" />
          DATA LOGIN ANDA
        </p>

        {/* Nomor Pendaftaran */}
        <div className="bg-white rounded-xl border border-brand-blue-200 p-4 mb-3 shadow-sm app-card">
          <p className="text-xs font-bold text-ink-500 mb-2 flex items-center gap-1.5 uppercase tracking-wider">
            <CreditCard className="w-3.5 h-3.5 text-brand-blue-600" />
            Nomor Pendaftaran
          </p>
          <div className="flex items-center justify-between gap-3">
            <p className="text-2xl font-black text-brand-blue-900 break-all bg-brand-blue-50/50 px-2 py-1 rounded-lg">
              {nomor_pendaftaran}
            </p>
            <button
              onClick={() => handleCopy(nomor_pendaftaran, "nomor")}
              className="p-2.5 bg-brand-blue-50 hover:bg-brand-blue-200 border border-brand-blue-200 rounded-xl transition-colors shrink-0 app-card hover:scale-105 active:scale-95"
        ...
        <div className="bg-white rounded-xl border border-brand-blue-200 p-4 mb-3 shadow-sm app-card">
          <p className="text-xs font-bold text-ink-500 mb-2 flex items-center gap-1.5 uppercase tracking-wider">
            <IdCard className="w-3.5 h-3.5 text-brand-blue-600" />
            NIK Santri (Password)
          </p>
          <div className="flex items-center justify-between gap-3">
        ...
        <div className="mt-4 py-2">
          <p className="text-xs text-ink-600 flex items-center justify-center gap-1.5 text-center font-medium">
            <AlertCircle className="w-3.5 h-3.5 shrink-0 text-brand-blue-600" />
      ...
      {/* Important Notes */}
      <div className="bg-white border-2 border-brand-blue-100 rounded-[1.5rem] p-5 mb-8 relative z-10 shadow-sm">
      ...
      <div className="relative z-10 w-full mb-6">
        <button
          ...
          className="w-full py-4 md:py-5 rounded-pill bg-brand-blue-600 text-white font-black text-lg hover:bg-brand-blue-700 shadow-xl shadow-brand-blue-200 transition-all flex items-center justify-center gap-2.5 active:scale-95"
        >
...
function LoadingFallback() {
  return (
    <div className="app-card max-w-lg w-full bg-white rounded-[2.5rem] shadow-lg border border-brand-blue-200 p-8">
...
export default function DaftarSuksesPage() {
  return (
    <main className="min-h-screen bg-white flex items-center justify-center p-6 relative overflow-hidden">
      {/* Background Decor */}
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-brand-blue-100/40 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/2 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-brand-blue-200/20 rounded-full blur-[100px] -translate-x-1/2 translate-y-1/2 pointer-events-none" />
