...
export default function LoginPage() {
  ...
  const handleLoginPendaftar = async (e: React.FormEvent) => {
    ...
    if (!/^[A-Z0-9]{5,15}$/i.test(nomorPendaftaran)) {
      setError("Format nomor pendaftaran tidak valid (pastikan hanya berisi huruf dan angka tanpa spasi)");
      setIsLoading(false);
      return;
    }
        ...
        >
          <Link href="/">
            <div className="app-card inline-flex items-center justify-center w-20 h-20 bg-white rounded-3xl shadow-sm border border-brand-blue-100 mb-8 hover:scale-110 transition-transform group overflow-hidden">
              <img src="/images/logo-andalus.webp" alt="Logo Ulul Albaab" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
            </div>
          </Link>
