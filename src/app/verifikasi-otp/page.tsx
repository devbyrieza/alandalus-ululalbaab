...
function VerifikasiOTPContent() {
  ...
  return (
    <div className="app-card max-w-md w-full bg-white rounded-[2.5rem] shadow-lg border border-blue-100 p-8 relative z-10">
      {/* Decorative Blur Inside Card */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-amber-50/30 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none" />
      ...
      <div className="text-center mb-6 relative z-10">
        ...
        >
          {/* Icon WhatsApp only */}
          <div className="p-3 bg-blue-700 rounded-2xl shadow-sm app-card">
        ...
        {!sim_code && (
          <p className="text-lg font-black text-blue-700">{no_hp}</p>
        )}

        {/* OTP Display Banner (when WhatsApp not available) */}
        {sim_code && (
          <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-xl app-card shadow-sm">
            <p className="text-xs font-black uppercase tracking-widest text-blue-800 mb-1">
              Kode Verifikasi Anda
            </p>
            <p className="text-sm text-blue-950 font-medium">
              Masukkan kode berikut:{" "}
              <span className="font-mono text-xl font-black bg-white px-2 py-0.5 rounded-lg border border-amber-100 shadow-sm ml-2">
                {sim_code}
              </span>
            </p>
            <p className="text-[10px] text-blue-600 mt-2 font-bold bg-blue-50 inline-block px-2 py-1 rounded-md">
            ...
            className={`w-12 h-14 md:w-14 md:h-16 text-center text-3xl font-display font-black border-2 rounded-xl focus:outline-none focus:ring-4 transition-all app-card shadow-sm ${
              otpError
                ...
                : digit
                  ...
                  : "border-blue-100 bg-white focus:ring-blue-50 focus:border-blue-300 text-ink-900"
            }`}
      ...
      <div className="relative z-10">
        ...
        {/* Loading State */}
        {isVerifying && (
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-xl app-card shadow-sm">
      ...
      <div className="space-y-3 relative z-10">
        ...
        <button
          ...
          className="w-full py-4 px-6 border-2 font-bold rounded-pill app-card transition-all duration-300 disabled:opacity-50 flex items-center justify-center gap-2 border-blue-200 text-blue-700 bg-white hover:bg-blue-50 active:scale-95"
        >
          <RefreshCw
        ...
        <button
          ...
          disabled={isVerifying}
          className="w-full py-3 px-6 text-ink-500 text-sm font-black uppercase tracking-widest hover:text-blue-700 flex items-center justify-center gap-2 disabled:opacity-50 transition-colors"
        >
      ...
      {/* Help Info */}
      <div className="mt-6 p-4 bg-blue-50 border border-blue-100 rounded-[1.5rem] relative z-10 app-card shadow-sm">
        ...
        <ul className="text-xs text-ink-600 space-y-1.5 font-medium pl-1">
          <li className="flex gap-2">
            <span className="text-blue-500 opacity-50">•</span> Pastikan
            nomor HP Anda aktif dan terdaftar di WhatsApp
          </li>
          <li className="flex gap-2">
            <span className="text-blue-500 opacity-50">•</span> Kode
            otomatis terverifikasi setelah 6 digit terisi
          </li>
          <li className="flex gap-2">
            <span className="text-blue-500 opacity-50">•</span> Jika tidak
      ...
      <div className="mt-5 text-center relative z-10">
        ...
        <a
          ...
          className="text-xs bg-amber-50 text-blue-700 px-4 py-1.5 rounded-full font-black border border-amber-200 hover:bg-amber-100 transition-colors inline-block"
        >
...
function LoadingFallback() {
  return (
    <div className="app-card max-w-md w-full bg-white rounded-[2.5rem] shadow-lg border border-blue-100 p-8">
...
export default function VerifikasiOTPPage() {
  return (
    <main className="min-h-screen bg-white flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-blue-50/30 rounded-full blur-[100px] -translate-x-1/2 translate-y-1/2 pointer-events-none" />
