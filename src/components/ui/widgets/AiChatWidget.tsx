...
export default function AiChatWidget({
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="bg-blue-800 text-white p-4 md:p-5 flex items-center justify-between shrink-0 shadow-sm relative z-10">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-5 bg-surface-50/50 flex flex-col gap-4">
            ...
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 mb-1 ${
                msg.role === "user"
                  ...
                  : "bg-blue-600 text-white shadow-premium-xs"
              }`}
            >
              {msg.role === "user" ? (
        ...
        {isLoading && (
          ...
          >
            <div className="w-8 h-8 rounded-full bg-blue-600 text-white flex items-center justify-center shrink-0 mb-1 shadow-premium-xs">
              <Bot className="w-5 h-5" />
            </div>
        ...
        {chatCount >= 3 && (
          ...
          >
            <button
              ...
              className="w-full bg-white border border-amber-200 text-amber-700 hover:bg-amber-50 hover:border-amber-300 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all shadow-premium-xs"
            >
      ...
      <div className="p-4 bg-white border-t border-surface-100 shrink-0">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-surface-50 border border-surface-200 rounded-xl px-4 py-3 text-[14px] text-ink-900 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-all placeholder:text-ink-400"
            disabled={isLoading}
          />
          <button
            onClick={handleSend}
            disabled={isLoading || !input.trim()}
            className="w-12 h-[46px] bg-blue-700 text-white rounded-xl flex items-center justify-center hover:bg-blue-800 disabled:opacity-50 disabled:hover:bg-blue-700 transition-colors shrink-0 shadow-sm"
          >
            {isLoading ? (
