...
export default function ChatSystem() {
  ...
  const openWhatsApp = () => {
    ...
    const waMessage = encodeURIComponent(
      "Halo Panitia PPDB Pesantren Al Andalus Ulul Albaab, saya ingin bertanya...",
    );
          ...
          <AnimatePresence>
            {!isAiOpen && (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="w-[60px] h-[60px] md:w-16 md:h-16 bg-primary-700 text-white rounded-[1.2rem] md:rounded-[1.3rem] flex items-center justify-center shadow-[0_10px_30px_rgba(30,58,138,0.4)] relative transition-all duration-300"
              >
                {/* Pulse Effect */}
                <span
