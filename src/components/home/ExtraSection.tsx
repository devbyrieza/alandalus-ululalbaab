...
const EXTRA_ACTIVITIES: ExtraItem[] = [
  ...
  { name: "Kaligrafi", icon: PenTool, accent: "amber" },
  { name: "Jurnalistik", icon: FileText, accent: "blue" },
  { name: "Konten Kreator", icon: Play, accent: "amber" },

  { name: "Basket", icon: Dumbbell, accent: "blue" },
  { name: "Bulutangkis", icon: Zap, accent: "amber" },
  { name: "Pertanian", icon: TreePine, accent: "amber" },
  { name: "Periklanan", icon: Waves, accent: "blue" },
  { name: "Web Programming", icon: Rocket, accent: "amber" },
] as const;
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3 md:gap-4 max-w-6xl mx-auto">
          {EXTRA_ACTIVITIES.map((item, idx) => (
            <motion.div
              ...
              className="bg-white p-6 rounded-lg border border-primary-100 flex flex-col items-center justify-center text-center group hover:border-primary-200 shadow-premium-sm hover:shadow-premium-md transition-all duration-400 relative overflow-hidden"
            >
