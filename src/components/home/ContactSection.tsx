        ...
        >
          ...
          <div className="relative z-10 flex flex-col lg:flex-row gap-12 lg:gap-16 items-center">
            ...
            <div className="lg:w-1/2 text-center lg:text-left">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-pill bg-white border border-cream-200 text-brand-blue-700 text-xs font-bold uppercase tracking-widest mb-6 shadow-sm">
            ...
            <div className="lg:w-1/2 grid sm:grid-cols-2 gap-4 sm:gap-6 w-full">
              {CONTACT_INFO.map((item, idx) => (
                <div key={idx} className={`app-card p-6 md:p-8 flex flex-col items-start group ${idx === 0 ? 'sm:col-span-2' : ''}`}>
                  <div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center mb-5 md:mb-6 transition-colors shadow-sm group-hover:scale-110 duration-500 shrink-0 ${item.color === 'blue' ? 'bg-brand-blue-50 text-brand-blue-600' :
