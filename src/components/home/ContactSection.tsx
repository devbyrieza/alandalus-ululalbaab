    ...
    >
      <Container>
        <motion.div
          ...
          className="bg-brand-blue-50/40 rounded-[2rem] lg:rounded-[2.5rem] p-8 md:p-12 lg:p-14 border border-brand-blue-100 relative overflow-hidden shadow-sm"
        >
          ...
          <div className="relative z-10 flex flex-col lg:flex-row gap-12 lg:gap-16 items-center">
            ...
            <div className="lg:w-[45%] text-center lg:text-left">
              <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white border border-brand-blue-200 text-brand-blue-700 text-[0.65rem] font-bold uppercase tracking-[0.12em] mb-5 shadow-xs">
            ...
            <div className="lg:w-[55%] grid sm:grid-cols-2 gap-3 sm:gap-4 w-full">
              {CONTACT_INFO.map((item, idx) => (
                <div
                  key={idx}
                  className={`bg-white p-6 rounded-2xl border border-brand-blue-100 shadow-premium-sm hover:shadow-premium-md flex flex-col items-start group transition-all duration-400 ${idx === 0 ? "sm:col-span-2" : ""}`}
                >
                  <div
                    className={`w-11 h-11 rounded-[12px] flex items-center justify-center mb-4 transition-transform shadow-xs group-hover:scale-105 duration-400 shrink-0 ${
                      item.accent === "blue"
                        ...
                        : item.accent === "yellow"
                          ? "bg-brand-yellow-50 text-brand-yellow-700"
                          : "bg-brand-blue-50 text-brand-blue-700"
                    }`}
                  >
                    <item.icon className="w-5 h-5" strokeWidth={1.8} />
                  </div>
                  <h3 className="text-ink-900 font-bold text-[0.9375rem] mb-1.5 tracking-tight group-hover:text-brand-blue-700 transition-colors">
                    {item.title}
                  </h3>
