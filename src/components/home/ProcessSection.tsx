...
import { UserPlus, FileText, CreditCard, ClipboardCheck, GraduationCap, CheckCircle2, BellRing, ArrowRight } from "lucide-react";
...
const ACCENT_MAP = {
    blue: {
        icon: "bg-brand-blue-50 text-brand-blue-600 border-brand-blue-100 group-hover:bg-brand-blue-100 group-hover:border-brand-blue-200",
        badge: "bg-brand-blue-600 text-white",
        title: "group-hover:text-brand-blue-700",
    },
    yellow: {
        icon: "bg-brand-yellow-50 text-brand-yellow-700 border-brand-yellow-100 group-hover:bg-brand-yellow-100 group-hover:border-brand-yellow-200",
        badge: "bg-brand-yellow-500 text-white",
        title: "group-hover:text-brand-yellow-700",
    },
};

// ─── Step Card ────────────────────────────────────────
function StepCard({ icon: Icon, title, description, accent, index, isLast }: (typeof STEPS)[number] & { index: number; isLast: boolean }) {
    ...
    return (
        <div className="relative flex flex-col items-center">
            ...
            >
                {/* Step number badge */}
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[0.6rem] font-black mb-4 shadow-xs ${colors.badge}`}>
                    {index + 1}
                </div>
                ...
                <h4 className={`font-bold text-[0.9375rem] text-ink-900 mb-2 tracking-tight transition-colors duration-200 ${colors.title}`}>
                    {title}
                </h4>
            ...
            {!isLast && (
                <div className="hidden lg:block absolute top-[28px] left-[calc(50%+44px)] right-[calc(-50%+44px)] h-px bg-gradient-to-r from-brand-yellow-200 to-brand-blue-100 z-0" />
            )}

            {/* Mobile vertical connector */}
            {!isLast && (
                <div className="lg:hidden mt-6 mb-2 w-px h-8 bg-gradient-to-b from-brand-blue-200 to-transparent rounded-full" />
            )}
        </div>
    );
}
...
export default function ProcessSection() {
    return (
        <section id="alur" className="section-alt relative border-y border-brand-yellow-100/60 overflow-hidden">
            ...
            <div
                ...
                style={{ background: "radial-gradient(circle, rgba(220,237,255,0.35) 0%, transparent 65%)" }}
            />
            ...
            <Container className="relative z-10">
                ...
                <div className="text-center mb-14 md:mb-18 max-w-2xl mx-auto">
                    ...
                    >
                        Ikuti langkah-langkah mudah berikut untuk menjadi bagian dari keluarga besar
