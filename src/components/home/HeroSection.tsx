...
import { ArrowRight, GraduationCap, Sparkles, Globe, CheckCircle2 } from "lucide-react";
...
export default function HeroSection() {
  ...
  return (
    <section
      ...
      style={{ background: "linear-gradient(160deg, var(--color-surface-50) 0%, var(--color-white) 55%, var(--color-brand-blue-50) 100%)" }}
    >
      {/* Atmospheric Background */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden" aria-hidden="true">
        <div className="absolute -top-[20%] -left-[10%] w-[55%] h-[65%] rounded-full opacity-35" style={{ background: "radial-gradient(ellipse, var(--color-brand-blue-200) 0%, transparent 70%)", filter: "blur(90px)" }} />
        <div className="absolute top-[10%] -right-[8%] w-[40%] h-[50%] rounded-full opacity-30" style={{ background: "radial-gradient(ellipse, var(--color-brand-yellow-300) 0%, transparent 70%)", filter: "blur(100px)" }} />
        <div className="absolute bottom-[-5%] left-[20%] w-[45%] h-[40%] rounded-full opacity-20" style={{ background: "radial-gradient(ellipse, var(--color-brand-blue-100) 0%, transparent 70%)", filter: "blur(80px)" }} />
        <div className="absolute inset-0 opacity-[0.022]" style={{ backgroundImage: `linear-gradient(var(--color-brand-blue-400) 1px, transparent 1px), linear-gradient(90deg, var(--color-brand-blue-400) 1px, transparent 1px)`, backgroundSize: "64px 64px" }} />
          ...
          <div className="flex flex-col gap-7 lg:gap-9 text-center lg:text-left">
            ...
            <motion.div variants={fadeUp} initial="hidden" animate={animate} transition={{ delay: 0.1 }} className="flex justify-center lg:justify-start">
              <span
                ...
                style={{ background: "var(--color-white)", border: "1px solid var(--color-brand-blue-200)", color: "var(--color-brand-blue-700)", boxShadow: "var(--shadow-sm), inset 0 1px 0 rgba(255,255,255,0.8)" }}
              >
                <Sparkles className="w-3.5 h-3.5 flex-shrink-0" style={{ color: "var(--color-brand-yellow-500)" }} aria-hidden="true" />
            ...
            <motion.div variants={fadeUp} initial="hidden" animate={animate} transition={{ delay: 0.2 }} className="space-y-3">
              <h1
                ...
                style={{ color: "var(--color-brand-blue-950)", fontSize: "clamp(2rem, 5vw + 0.75rem, 5rem)" }}
              >
                <span className="block">Kaderisasi Ummat</span>
                <span
                  className="block mt-1"
                  style={{ background: "linear-gradient(135deg, var(--color-brand-blue-900) 0%, var(--color-brand-yellow-600) 50%, var(--color-brand-blue-950) 100%)", WebkitBackgroundClip: "text", backgroundClip: "text", WebkitTextFillColor: "transparent", paddingBottom: "0.15em" }}
                >
                  Rabbani, Cendekia,{" "}
            ...
            {/* Body Copy */}
            <motion.p
              variants={fadeUp} initial="hidden" animate={animate} transition={{ delay: 0.3 }}
            ...
            >
              ...
              <strong className="font-bold" style={{ color: "var(--color-brand-blue-800)" }}>Kitab Turots, Tahfizh Al-Qur'an, dan Entrepreneurship</strong>{" "}
            ...
            <motion.div variants={fadeUp} initial="hidden" animate={animate} transition={{ delay: 0.38 }} className="flex items-center gap-3 justify-center lg:justify-start">
              <div className="h-px flex-1 max-w-[3rem]" style={{ background: "var(--color-brand-blue-200)" }} />
              <p className="text-sm font-semibold italic" style={{ color: "var(--color-brand-blue-700)" }}>
            ...
            <motion.div variants={fadeUp} initial="hidden" animate={animate} transition={{ delay: 0.45 }} className="flex flex-col gap-4 items-center lg:items-start">
              <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                <Link href="/ppdb" className="w-full sm:w-auto">
                  <button className="btn-primary w-full sm:w-auto px-8 lg:px-10 py-4 lg:py-[1.125rem] min-h-[56px] text-[0.9375rem]" style={{ boxShadow: "var(--shadow-blue-lg)" }}>
              ...
              <div className="flex items-center gap-3 mt-1">
                <div className="flex -space-x-2.5">
                  {[{ bg: "var(--color-brand-blue-200)" }, { bg: "var(--color-brand-yellow-200)" }, { bg: "var(--color-brand-blue-300)" }, { bg: "var(--color-brand-yellow-300)" }].map((item, i) => (
                ...
                <p className="text-[11px] font-semibold leading-tight" style={{ color: "var(--color-ink-500)" }}>
                  <span className="font-bold uppercase tracking-wide" style={{ color: "var(--color-brand-blue-600)" }}>Angkatan ke-4</span>
                  {" • "}Managed by Al Andalus IIBS
                </p>
              </div>

              <div className="flex flex-wrap gap-x-5 gap-y-2 justify-center lg:justify-start mt-1">
                {["Putra 32 • Putri 30 Kuota", "Bahasa Arab aktif", "Tahfizh + Entrepreneurship"].map((point) => (
                  <span key={point} className="flex items-center gap-1.5 text-[11px] font-semibold" style={{ color: "var(--color-ink-500)" }}>
                    <CheckCircle2 className="w-3.5 h-3.5 flex-shrink-0" style={{ color: "var(--color-brand-blue-500)" }} aria-hidden="true" />
                    {point}
                  </span>
                ))}
              </div>
          ...
          >
            ...
            >
              <div className="flex items-center gap-3 px-4 py-3 rounded-2xl" style={{ background: "rgba(255,255,255,0.95)", backdropFilter: "blur(20px) saturate(1.8)", WebkitBackdropFilter: "blur(20px) saturate(1.8)", border: "1px solid rgba(255,255,255,0.7)", boxShadow: "var(--shadow-premium-md)" }}>
                <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "var(--color-brand-blue-50)" }}>
                  <GraduationCap className="w-5 h-5" style={{ color: "var(--color-brand-blue-600)" }} aria-hidden="true" />
                </div>
                <div>
                  <p className="text-[9px] font-bold uppercase tracking-[0.08em] leading-none mb-1" style={{ color: "var(--color-ink-400)" }}>Tersedia</p>
                  <p className="text-sm font-black leading-tight" style={{ color: "var(--color-brand-blue-900)" }}>MTs &amp; IL</p>
                  <p className="text-[10px] font-semibold mt-0.5" style={{ color: "var(--color-ink-500)" }}>Putra 32 · Putri 30</p>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center gap-3 px-4 py-3 rounded-2xl" style={{ background: "rgba(255,255,255,0.95)", backdropFilter: "blur(20px) saturate(1.8)", WebkitBackdropFilter: "blur(20px) saturate(1.8)", border: "1px solid rgba(255,255,255,0.7)", boxShadow: "var(--shadow-premium-md)" }}>
                <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "var(--color-brand-blue-50)" }}>
                  <Globe className="w-5 h-5" style={{ color: "var(--color-brand-blue-600)" }} aria-hidden="true" />
                </div>
                <div>
                  <p className="text-sm font-black leading-tight" style={{ color: "var(--color-brand-blue-900)" }}>Jaringan Global</p>
                  <p className="text-[10px] font-semibold mt-0.5" style={{ color: "var(--color-ink-500)" }}>Timur Tengah</p>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              style={{ background: "linear-gradient(135deg, var(--color-brand-yellow-400) 0%, var(--color-brand-yellow-600) 100%)", padding: "1rem 1.125rem", borderRadius: "1.5rem", border: "4px solid var(--color-white)", boxShadow: "var(--shadow-premium-lg)", transition: "transform 0.5s cubic-bezier(0.16, 1, 0.3, 1)" }}
            >
              <div className="text-center min-w-[80px]">
                <p className="text-[9px] font-black uppercase tracking-[0.1em] leading-none mb-1.5" style={{ color: "var(--color-brand-blue-950)" }}>Info PPDB</p>
                <p className="text-base font-black leading-tight" style={{ color: "var(--color-brand-blue-900)" }}>Pendaftaran<br />Dibuka</p>
                <div className="mt-2 py-1 px-2.5 rounded-full" style={{ background: "rgba(1, 30, 66, 0.12)" }}>
                  <p className="text-[9px] font-bold" style={{ color: "var(--color-brand-blue-900)" }}>Kuota Terbatas</p>
                </div>
              </div>
            </motion.div>

            {/* Decorative blobs */}
            <div className="absolute -z-10 -bottom-14 -right-14 w-64 h-64 rounded-full pointer-events-none" style={{ background: "radial-gradient(ellipse, var(--color-brand-blue-300) 0%, transparent 70%)", filter: "blur(40px)", opacity: 0.18 }} aria-hidden="true" />
            <div className="absolute -z-10 -top-10 -left-10 w-48 h-48 rounded-full pointer-events-none" style={{ background: "radial-gradient(ellipse, var(--color-brand-yellow-300) 0%, transparent 70%)", filter: "blur(36px)", opacity: 0.22 }} aria-hidden="true" />
          </motion.div>
